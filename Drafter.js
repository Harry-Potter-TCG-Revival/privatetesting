...
document.getElementById("Start_Solo_Draft_Button").addEventListener("click", () => {
    ...
    // Step 5: Display each card in the human player's "solo current pack"
    displaySoloPack();
});
...
function createCardPacks() {
    ...
    Object.keys(soloPlayers).forEach(playerKey => {
        for (let packNumber = 1; packNumber <= 3; packNumber++) {
            ...
            // Add 1 rare, 3 uncommons, and 7 commons to the pack
            pack.push(soloGetRandomCard(rareCards).name);
            for (let i = 0; i < 3; i++) pack.push(soloGetRandomCard(uncommonCards).name);
            for (let i = 0; i < 7; i++) pack.push(soloGetRandomCard(commonCards).name);
...
// Function to get a random card from the corresponding slot's rarity list
function soloGetRandomCard(cards) {
...
function loadRoundPack(roundNumber) {
    ...
    console.log(`Loaded packs for round ${roundNumber}:`, soloPlayers);
}
...
function soloCreateCardElement(card) {
    ...
    cardElement.addEventListener("click", () => {
        soloChooseCard(card, cardElement); // Call soloChooseCard with the card and its element
    });
...
function soloChooseCard(card) {
    ...
    if (cardIndex !== -1) {
        ...
        // Debug: Verify the updated pool and pack
        console.log("Updated Pool:", currentPool);
        console.log("Updated Pack:", currentPack);
    } else {
        console.warn("Card not found in the pack:", card.name);
    }


    console.log(`Card moved to pool: ${card.name}`);
    console.log(`Updated Current Pack:`, currentPack);
    console.log(`Updated Card Pool:`, currentPool);
...
function soloCompPicks() {
    Object.keys(soloPlayers).forEach(playerKey => {
        ...
        console.log(`${playerKey}'s current pack:`, currentPack);
        ...
        console.log(`${playerKey}'s card values:`, cardValues);
        ...
        console.log(`${playerKey} picks:`, bestPick.name);
        ...
        if (cardIndex !== -1) {
            ...
            console.log(`Updated Pool for ${playerKey}:`, player.soloCurrentPool);
            console.log(`Updated Pack for ${playerKey}:`, currentPack);
        } else {
...
// Evaluate the cards type against the player card pool typing.
function soloDraftTypeSynergy(card, playerPool) {
    ...
    console.log(`Card Type: ${cardType}, Type Count: ${cardTypeCount}, Type Percentage: ${typePercentage.toFixed(2)}, Weight Factor: ${weightFactor.toFixed(2)}, Synergy Value: ${synergyValue.toFixed(2)}`);
    return synergyValue;
}

//Card Curve evaluation
function evaluateCurve(card, playerPool) {
    ...
    console.log(`Card: ${card.name}, Card Cost: ${cardCost}, Average Cost: ${averageCost.toFixed(2)}, Weight Factor: ${weightFactor.toFixed(2)}, Adjustment: ${adjustment.toFixed(2)}`);
    return adjustment;
}
...
function soloRotatePacks() {
    ...
    console.log(`Packs rotated for ${isOddRound ? 'odd' : 'even'} round.`);
...
function soloEndOfRound() {
    ...
    if (soloRoundNumber >= 3) {
        ...
        console.log("Loaded round packs, now just display packs");

        // Then display the pack
        displaySoloPack();

        console.log("Player pack should be displayed now");
    }
}
...
function printSoloDraftResults() {
    ...
    // Iterate through each seat/player
    for (let i = 1; i <= 8; i++) {
        draftResults += `Player ${i} Results:\n`;

        // Add each pack
        for (let j = 1; j <= 3; j++) {
            draftResults += `Pack ${j}:\n`;
            const pack = soloSeats[`soloPack${j}`][i - 1]; // Access the specific pack
            pack.forEach(card => {
                draftResults += `  - ${card.name}\n`;
            });
        }

        // Add the card pool
        draftResults += `Card Pool:\n`;
        const pool = soloSeats.soloCurrentPool[i - 1];
        pool.forEach(card => {
            draftResults += `  - ${card.name}\n`;
        });
    ...
    console.log("Draft results saved as SoloDraftResults.txt");
}
